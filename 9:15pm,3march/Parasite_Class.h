...
public:
    ...
    std::array<double,2> getCoords_of(size_t index); //get coords for a specific parasite in {x,y} format

    void generateCoords_Draw(FILE* plot); //generate new coordinates and drawings

    int deduct_Health(int immunity_level, int surv_current_hp);
