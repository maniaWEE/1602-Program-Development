...
void Survivor::SurvivorSetUp(){
    //this only gonna be runned once in program
...
void Survivor::movesurvivor(FILE* plot, int id){
    static std::random_device rd;         // Step 1: random seed
    static std::mt19937 gen(rd());        // Step 2: engine
...
int Survivor::detect_Hit(std::vector<std::array<double,2>> coords_list, double radius){
    ...
    for (size_t i = 0; i < coords_list.size(); i++){
        ...
        double dist_between = std::sqrt( std::pow( (coords_of_survivor[0]-coords_list[i][0]) , 2)
                                       + std::pow( (coords_of_survivor[1]-coords_list[i][1]) , 2) ) - 2;
        if ( (-radius <= dist_between) && (dist_between <= radius) ){
            return i; // return the index(id-1) of that specific entity (useful only for stalker), function immediately finishes
        }
    }
